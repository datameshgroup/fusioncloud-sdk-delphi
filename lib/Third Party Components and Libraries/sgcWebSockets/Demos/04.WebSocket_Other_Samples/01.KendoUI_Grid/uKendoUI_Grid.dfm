object frmServerPROTOCOL: TfrmServerPROTOCOL
  Left = 0
  Top = 0
  Caption = 'WebSockets Server KendoUI Grid Sample'
  ClientHeight = 412
  ClientWidth = 531
  Color = clBtnFace
  ...
  object pageKendoUI_Grid: TPageProducer
    HTMLDoc.Strings = (
      '<!DOCTYPE html>'
      '<html>'
      '<head>'
      ...
      #9'<meta http-equiv="pragma" content="no-cache" />'
        ...
        'e=1">'
      #9'<title>WebSockets KendoUI Grid Demo</title>'
      ...
      '    <link href="kendo.silver.mobile.min.css" rel="stylesheet" />'
      ...
      '</head> '
      '<body> '
      '<div data-role="page" id="wsdemo_kendoui_gridl">'
      ''
      #9'<div data-role="header" data-theme="b">'#9
      #9#9'<h1>KendoUI Grid Demo</h1>'
      ...
      '        <h4>Information</h4>'
        ...
        '        <p>This example demonstrates real-time push-notification' +
        's from a WebSocket. To see the real-time updates:</p>'
      '        <ol>'
      
        '            <li>Open this page in another browser window by clic' +
        'king <a href="." target="_new">here</a></li>'
      '            <li>Create, update or destroy grid items.</li>'
      '        </ol>'
      
        '        <p><strong>Note:</strong> To further enhance real-time e' +
        'xperience, the Grid will also display changes from other users,'
      
        '            which currently happen to perform CRUD operations on' +
        ' the same demo.'
      
        '            If you refresh the page, all changes will be lost, b' +
        'ecause the remote service only broadcasts changes, it does not a' +
        'ctually save them.</p>'
      '    </div>'
      '    <div id="example">'
      '    <div id="grid"></div>'
      ...
      ''
      '        $(function(){'
      '            var host = '#39'ws://<#host>:<#port>'#39';'
      '            var ws = new sgcWebSocket(host);'
      ''
      '            //Bind the grid when the socket connects'
      '            ws.on('#39'open'#39', function(event) {'
      #9'document.getElementById('#39'status'#39').innerHTML = '#39'Socket Open'#39';'
      #9'document.getElementById('#39'status'#39').className = '#39'online'#39';'
      ''
      '                $("#grid").data("kendoGrid").dataSource.fetch();'
      '            });'
      ''
      '            ws.on('#39'close'#39', function(event) {'
      #9'document.getElementById('#39'status'#39').innerHTML = '#39'Socket Closed'#39';'
      #9'document.getElementById('#39'status'#39').className = '#39'offline'#39';'
      '            });'
      ''
      ...
      '            function send(ws, request, callback) {'
        ...
        '                //Assign unique id to the request. Will use that' +
        ' to distinguish the response.'
      '                request.id = kendo.guid();'
      ''
      
        '                //Listen to the "message" event to get server re' +
        'sponse'
      
        '                ws.websocket.addEventListener("message", functio' +
        'n(e) {'
      '                    var result = JSON.parse(e.data);'
      ''
      
        '                    //Check if the response is for the current r' +
        'equest'
      '                    if (result.id == request.id) {'
      '                        //Stop listening'
      
        '                        ws.websocket.removeEventListener("messag' +
        'e", arguments.callee);'
      ''
      ...
      '                dataSource: {'
        ...
        '                    // Handle the push event to display notifica' +
        'tions when push updates arrive'
      '                    push: function(e) {'
      
        '                        var notification = $("#notification").da' +
        'ta("kendoNotification");'
      ...
      '                        push: function(options) {'
        ...
        '                            //Listen to the "message" event fire' +
        'd when the server pushes data'
      '                            ws.on('#39'message'#39', function(event) {'
      
        '                                var result = JSON.parse(event.me' +
        'ssage);'
      ''
      
        '                                //Check what the push type is an' +
        'd invoke the corresponding callback.'
      ...
      '</body>'
      '</html>')
    OnHTMLTag = pageKendoUI_GridHTMLTag
    Left = 88
    Top = 48
  end
