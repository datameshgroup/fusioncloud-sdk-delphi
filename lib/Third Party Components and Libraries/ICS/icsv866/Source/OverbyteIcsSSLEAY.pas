...
const
    IcsSSLEAYVersion   = 866;
    CopyRight : String = ' IcsSSLEAY (c) 2003-2021 F. Piette V8.66 ';
...
var
    ...
    GLIBEAY_300DLL_Name         : String  =
            {$IFDEF POSIX}'/usr/lib/libcrypto.dylib';{$ELSE}   { !!!! not tested, unknown file name }
                {$IFDEF CPUX64}'libcrypto-3_0-x64.dll';{$ELSE}'libcrypto-3_0.dll';{$ENDIF} {$ENDIF}     { V8.65 }
    ...
    GSSLEAY_300DLL_Name         : String  =
            {$IFDEF POSIX}'/usr/lib/libssl.dylib';{$ELSE}
                {$IFDEF CPUX64}'libssl-3_0-x64.dll';{$ELSE}'libssl-3_0.dll';{$ENDIF}{$ENDIF}   { V8.65 }
    {$IFDEF YuOpenSSL}
...
{ V8.66 1.0.2 and 1.1.0 support ceased Dec 2019, now removed from ICS }
    ...
    OSSL_VER_3000D  = $30000000; // 3.0.0 dev                 { V8.60 }
    OSSL_VER_30000  = $3000000F; // 3.0.0 base                { V8.65 }
...
const
    GSSLEAYImports1: array[0..157] of TOSSLImports = (
    ...
    (F: @@SSL_get_peer_certificate;               N: 'SSL_get_peer_certificate';                  MI: OSSL_VER_MIN; MX: OSSL_VER_MAX),
...
{$IFNDEF YuOpenSSL}
 ...
 { V8.65 see if LibeayLoad so we report that error again and not this unit }
    if ICS_OPENSSL_VERSION_NUMBER = 999then begin
...
{ V8.65 open SSL DLL in same directory as LIBEAY DLL }
    ...
    if ICS_OPENSSL_VERSION_NUMBER >= OSSL_VER_3000D then
        GSSLEAY_DLL_FileName := GSSLEAY_DLL_FileName + GSSLEAY_300DLL_Name
  { else if ICS_OPENSSL_VERSION_NUMBER < OSSL_VER_1100 then
        GSSLEAY_DLL_FileName := GSSLEAY_DLL_FileName + GSSLEAY_DLL_Name }
    else
