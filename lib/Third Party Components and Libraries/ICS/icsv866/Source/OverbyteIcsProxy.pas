...
const
    ...
    CopyRight : String = ' TIcsHttpProxy (c) 2021 F. Piette V8.66 ';
    DefServerHeader : string = 'Server: ICS-Proxy-8.66';
...
begin
  ...
  { send header and body, V8.65 allow to remove passwords before logging }
    ...
    if LogHeader <> '' then
      LogTarEvent('Sending Response Header:' + cCRLF + LogHeader);
    SourceHdrRespXmit;
...
begin
    ...
    if P <= 2 then Exit;
    Q := IcsPosEx(cCRLF, Headers, P);                                           { V8.65 }
    if Q > P then begin
...
begin
    ...
    P := Pos(cCRLF + Hdr, Headers) + 2;
    if P <= 2 then begin
        Q := IcsPosEx(cDoubleCRLF, Headers, P);                                 { V8.65 }
             ...
             { V8.65 allow modify data before it's logged to remove passwords }
                ...
                if LogHeader <> '' then
                    LogTarEvent('Original Request Header: #' +
                              IntToStr(FHttpTotReqs) + cCRLF + LogHeader);
            end
            else if (FProxySource.DebugLevel >= DebugConn) then
        ...
        { V8.65 allow modify data before it's logged to remove passwords }
            ...
            if LogHeader <> '' then
                LogTarEvent('Modified Response Header: #' +
                            IntToStr(FHttpTotResps) + cCRLF + LogHeader);
        end;
        SourceHdrRespXmit;
             ...
             { V8.65 allow modify data before it's logged to remove passwords }
                ...
                if LogHeader <> '' then
                    LogTarEvent('Original Response Header: #' + IntToStr(FHttpTotResps) +
                        ', for Request: ' + FHttpInfo [FHttpCurrResp].ReqStartLine + cCRLF + LogHeader);
            end
            else if (FProxySource.DebugLevel >= DebugConn) then
         ...
         { note Transfer-Encoding: chunked and Content-Encoding: gzip is common, uncompress after unchunk }
          ...
          { may need to change Location: header http<>https and or fix host }
            if (FRespLocation <> '') and FUpdateHttp  then begin
                TargetCheck;  { V8.65 need to initialize on }
