...
Dec 09, 2020 V8.65 Only increase TCP buffer size, don't reduce it below default
...
const
  FtpCliVersion      = 866;
  CopyRight : String = ' TFtpCli (c) 1996-2021 F. Piette V8.66 ';
  FtpClientId : String = 'ICS FTP Client V8.66 ';   { V2.113 sent with CLNT command  }
...
type
  ...
  protected
    ...
    FStartTime          : Integer;    { V8.65 }
    FStopTime           : Integer;    { V8.65 }
    FState              : TFtpState;
    FStatusCode         : Integer;    { V8.65 }
    ...
    FStatusCodeSave     : Integer;    { V8.65 } { To save FStatusCode when quitting }
...
{$ENDIF}
  ...
  protected
    ...
    FTimeStop      : Integer;    { V8.65 }    { Milli-seconds    }
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function GetInteger(Data : PChar; var Number : Integer) : PChar;     { V8.65 }
var
    bSign : Boolean;
begin
    Number := 0;
...
{$ENDIF}
    ...
    FStartTime := Integer(IcsGetTickCount);     { V8.65 }
...
begin
    FStopTime := Integer(IcsGetTickCount);     { V8.65 }
...
begin
    ...
    {$ENDIF}
        ...
        if not (FFctPrv in [ftpFctSiteCmlsd, ftpFctXCmlsd, ftpFctSiteIndex]) then begin
            ...
            if Length(FLastMultiResponse) < 65536 then
...
begin
    ...
    if FTimeout > 0 then
        FTimeStop := Integer(IcsGetTickCount) + FTimeout * 1000;    { V8.65 }
end;
...
{$ENDIF}
  ...
  {$ENDIF}
    while TRUE do begin
        ...
        if Terminated or
           ((FTimeout > 0) and (Integer(IcsGetTickCount) > FTimeStop)) then begin    { V8.65 }
            { Timeout occured }
            AbortAsync;
...
//       (SSL_get_state(FControlSocket.Ssl) = TLS_ST_OK) then
