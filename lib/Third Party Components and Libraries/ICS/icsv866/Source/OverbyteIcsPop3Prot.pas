...
Jul 9, 2018  V8.56 Added OverbyteIcsTypes
Dec 09, 2020 V8.65 Linux pending PrepareGlobalSmtpFormatSettings.
                   Added popAuthXOAuth2 and popAuthOAuthBearer OAuth2
                     authentication, not automatically selected, access token is
                     specified in AuthBearer property with it's expiry date in
                     BearerExpireDT. These are checked in the Connect method and
                     event onGetNewToken called if invalid which should then call
                     TIcsRestEmail.GetNewToken which will use OAuth2 to launch a
                     browser window and request login and password, and then
                     update the access token. A refresh token will usually also
                     be returned which should be saved and will be used by GetNewToken
                     to avoid the browser window if still valid (usually for months).
                     Note the Password property is ignored for OAuth2 methods, but
                     the username should match that selected in the browser and
                     is encoded with the token in the XOAUTH2 or XOAUTHBEARER
                     base64 argument. Goggle uses popAuthXOAuth2 with a single
                     line AUTH command, Microsoft uses popAuthXOAuth22 with two
                     lines.  See notes about TIcsRestEmail for more information.
...
const
    Pop3CliVersion     = 865;
    CopyRight : String = ' POP3 component (c) 1997-2020 F. Piette V8.65 ';
...
type
    ...
    TPop3AuthType = (popAuthNone,    popAuthLogin,
                     ...
                     popAuthXOAuth2, popAuthOAuthBearer);  { V8.65 two OAuths }
    ...
    {    FReceiveBuffer      : array [0..POP3_RCV_BUF_SIZE - 1] of AnsiChar;  V8.06 no longer used }
        ...
        FOnGetNewToken      : TNotifyEvent;                            { V8.65 for OAuth2 }
...
{$ENDIF}
    public
        ...
        property OnGetNewToken     : TNotifyEvent    read  FOnGetNewToken
                                                     write FOnGetNewToken;     { V8.65 for OAuth2 }
...
{$ENDIF}
    ...
    published
        ...
        property OAuthToken;         { V8.65 for OAuth2 }
        property TokenExpireDT;      { V8.65 for OAuth2 }
        property OnGetNewToken;      { V8.65 for OAuth2 }
        property Tag;
...
{ V8.65 some literals for applications }
    PopAuthTypeNames: array [TPop3AuthType] of PChar =
        ('None','Login','Cram-Md5','Cram-Sha1','NTLM','XOAuth2','OAuthBearer');
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{ OAuth error response is Json base64 encoded }
procedure TCustomPop3Cli.AuthNextOAuth;                                          { V8.65 }
var
    AResponse: AnsiString;
begin
 // SASL response on second line
...
begin
    ...
    case FAuthType of
     ...
     { OAuth2 used by gmail and Microsoft, token sent on second line  }
        popAuthXOAuth2 : ExecAsync(pop3Auth, 'AUTH XOAUTH2',
                                               pop3Transaction, AuthNextOAuth); { V8.65 }
...
begin
  ...
  { V8.65 if using OAuth2 authentication, check and update Bearer token }
  { no point in continuing without it }
    if FAuthType in [popAuthXOAuth2, popAuthOAuthBearer] then begin
        if (FOAuthToken = '') or (FTokenExpireDT < Now) then begin
            if Assigned(FOnGetNewToken) then
                FOnGetNewToken(Self);  { application should get new Token }
            if (FOAuthToken = '') then
                raise Pop3Exception.Create('POP3 needs OAuth2 authentication token');
            if (FTokenExpireDT < Now) then
                raise Pop3Exception.Create('OAuth2 authentication token has expired and must be refreshed');
        end;
    end;
