...
const
    HttpCliVersion       = 866;
    CopyRight : String   = ' THttpCli (c) 1997-2021 F. Piette V8.66 ';
    DefaultProxyPort     = '80';
...
{$ENDIF}
    ...
    THttpAuthType    = (httpAuthNone, httpAuthBasic, httpAuthNtlm,
                        ...
                        httpAuthJWT, httpAuthOAuth);  { V8.62, V8.65 }
...
{$IFNDEF NO_DEBUG_LOG}
    if CheckLogOptions(loProtSpecInfo) then  { V1.91 }
        DebugLog(loProtSpecInfo, 'SessionConnected, error='  +
          IntToStr (ErrCode) + ' to ' + IcsFmtIpv6AddrPort(FAddrResolvedStr, FPort));  { V8.60 }
{$ENDIF}
    if ErrCode <> 0 then begin
        FRequestDoneError := ErrCode;
        FStatusCode       := 404;
        FReasonPhrase     := WSocketErrorMsgFromErrorCode(ErrCode) +  { V8.51 handles proxy errors }
                             ' (Error #' + IntToStr(ErrCode) + ') to ' +
                               IcsFmtIpv6AddrPort(FAddrResolvedStr, FPort);  { V8.60 }
        FLastAddrOK := '';  { V8.60 }
