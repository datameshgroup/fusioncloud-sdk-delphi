...
const
  WebServVersion     = 865;
  CopyRight : String = 'WebServ (c) 1999-2020 F. Piette V8.65 ';
  ...
  UPLOAD_DIR         = {$IFDEF MSWINDOWS} 'upload\' {$ELSE} 'upload/' {$ENDIF};
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TWebServForm.WndProc(var Msg: TMessage);
begin
    try
...
begin
    ...
    FDataFileName := ExtractFilePath(FIniFileName) + 'FormHandler.txt';
    if not FileExists(FDataFileName) then
    begin
      hFile := FileCreate(FDataFileName);
...
begin
  {$IFDEF MSWINDOWS}
    HttpServer1.OnAuthNtlmBeforeValidate := HttpServer1AuthNtlmBeforeValidate;
  {$ENDIF}
    if DirListCheckBox.IsChecked then
        HttpServer1.Options := HttpServer1.Options + [hoAllowDirList]
    else
        HttpServer1.Options := HttpServer1.Options - [hoAllowDirList];
    if OutsideRootCheckBox.IsChecked then
...
begin
    { Open data file                                                        }
    AssignFile(ClientCnx.FDataFile, FDataFileName);
...
var
    I : Integer;
begin
    if not DisplayHeaderCheckBox.IsChecked then
        Exit;
...
begin
    if WriteLogFileCheckBox.IsChecked then
        OpenLogFile
    else
        CloseLogFile;
    WriteLogFileCheckBox.IsChecked := FLogFileOpened;
...
begin
  ...
  {$ENDIF}
        ...
        ClientCnx.AuthTypes  := [atBasic, atDigest {$IFDEF MSWINDOWS}, atNtlm {$ENDIF}];
