...
begin
    ...
    FPublicKey := TX509Base.Create(self);
...
begin
  ...
  //      TestSignDigest(AnsiData, TestPrivKeyFile.Text, 'sha256');
        ...
        FPublicKey.PublicKeyLoadFromText(PublicKeyStr);

        Flag := IcsAsymVerifyDigest(AnsiData, Signature, FPublicKey.PrivateKey, Digest_sha256);
...
begin
    ...
    // get public key from Json Web Signature
        if JoseAlg >= jsigRsa256 then  begin
            ...
            FPublicKey.PrivateKey := IcsJoseJWKGetPKey(JwkPub, NewKeyid);
            if Assigned(FPublicKey.PrivateKey) then begin
                if ShowRawKey.Checked then begin
                    AddLog (FPublicKey.GetPKeyRawText(NOT JwkPrivate.Checked));
                end;

          //    if FPublicKey.PublicKeySaveToText = FPrivateKey.PublicKeySaveToText then
                Ret := EVP_PKEY_cmp(FPublicKey.PrivateKey, FPrivateKey.PrivateKey);
                if Ret = 1 then
                     AddLog ('JWK public key matches original private key')
                 else
                     AddLog ('Error, JWK public key does not match private key, result ' + IntToStr(ret));
...
begin
    ...
    try
        ...
        if (JoseAlg >= jsigRsa256) or (KeyType <> JoseLitkty_oct) then  begin
            AddLog ('Getting public key from JWK');
            FPublicKey.PrivateKey := IcsJoseJWKGetPKey(RawJWK.Lines.Text, NewKeyid);
            if Assigned(FPublicKey.PrivateKey) then begin
                if ShowRawKey.Checked then begin
                    AddLog (FPublicKey.GetPKeyRawText(True));
                end;
                AddLog ('JWK public key found: ' + FPublicKey.PrivateKeyInfo);
            end
            else
                  AddLog ('No JWK public key found');
        end
...
var
    ...
    begin
        NewKeyid := MyKid;
        NewPubKey := FPublicKey.PrivateKey;
     ...
     // need to save new public key
        if VerifyRes = JVerifyOkNewPubKey then begin
            FPublicKey.PrivateKey := NewPubKey;
            AddLog('Saved JWK: ' + Trim(FPublicKey.PublicKeySaveToText));
        end;
...
begin
    ...
    try
        ...
        // V8.65 keep pubLic key for verify without JWK
            FPublicKey.PublicKeyLoadFromText(FPrivateKey.PublicKeySaveToText);
            AddLog ('Public Key: ' + FPublicKey.PrivateKeyInfo);
            AddLog ('');
        end;
