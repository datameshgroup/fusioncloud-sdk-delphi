...
Nov 05, 2020 V8.65 - Linux pending PrepareGlobalSmtpFormatSettings.
                     Added smtpAuthXOAuth2 and smtpAuthOAuthBearer authentication,
                       not automatically selected, access token is specified in
                       AuthBearer property with it's expiry date in BearerExpireDT.
                       These are checked in the Connect method and event onGetNewToken
                       called if invalid which should then call TIcsRestEmail.GetNewToken
                       which will use OAuth2 to launch a browser window and request
                       login and password, and then update the access token. A
                       refresh token will usually also be returned which should be
                       saved and will be used by GetNewToken to avoid the browser
                       window if still valid (usually for months).  Note the
                       Password property is ignored for OAuth2 methods, but
                       the username should match that selected in the browser and
                       is encoded with the token in the XOAUTH2 or XOAUTHBEARER
                       base64 argument.
                       See notes about TIcsRestEmail for more information.
                     Added SmtpAuthTypeNames literals for applications.
                     Published onBeforeOutStreamFree so we can read stream
                       before it is destroyed.
                     THtmlSmtpCli descends from TSslSmtpCli so more properties
                      are published.
...
const
  SmtpCliVersion     = 865;
  CopyRight : String = ' SMTP component (c) 1997-2020 Francois Piette V8.65 ';
...
//        property    OnBeforeOutStreamFree; {AG} { V8.65 now published }
    ...
    published
        ...
        property OnBeforeOutStreamFree; { V8.65 was public }
        property XMailer;
        property ProxyType;
...
//    THtmlSmtpCli = class(TCustomSslSmtpCli)
    THtmlSmtpCli = class(TSslSmtpCli)  { V8.65 why custom ? }
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
(* this command implements various SASL (Simple Authentication and Security
    Layer)  mechanisms  *)
...
procedure TCustomSmtpClient.AuthNextOAuth;                             { V8.65 }
var
    Response: String;
begin
  ...
  { no point in continuing without it }
    if FAuthType in [smtpAuthXOAuth2, smtpAuthOAuthBearer] then begin
        if (FOAuthToken = '') or (FTokenExpireDT < Now) then begin
            ...
            if (FOAuthToken = '') then
                raise SmtpException.Create('SMTP needs OAuth2 authentication token');
            if (FTokenExpireDT < Now) then
                raise SmtpException.Create('OAuth2 authentication token has expired and must be refreshed');
        end;
    end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF NEVER}
(* ion Rfc822DateTime(t : TDateTime) : String;
var
    Year, Month, Day     : WORD;
