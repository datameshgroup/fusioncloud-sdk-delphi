...
{$IFNDEF YuOpenSSL}
       ...
       //     pctx := EVP_PKEY_CTX_new_id(EVP_PKEY_RSA, Nil);
            if (pctx = Nil) then
                RaiseLastOpenSslError(EDigestException, FALSE, 'Failed to create RSA context');
            try
                if EVP_PKEY_fromdata_init(pctx) <> 1 then
                    RaiseLastOpenSslError(EDigestException, FALSE, 'Failed to init RSA parameters');
                ret := EVP_PKEY_fromdata(pctx, @Result, ctype, ParamArray);
                if ret = -2 then
                    Raise EDigestException.Create('Parameters not supported for RSA keys')
                else if ret < 1 then
                    RaiseLastOpenSslError(EDigestException, FALSE, 'Failed to set RSA params');
            finally
...
{$IFNDEF YuOpenSSL}
       ...
       //     pctx := EVP_PKEY_CTX_new_id(nid, Nil);
            ...
            try
                if EVP_PKEY_fromdata_init(pctx) <> 1 then
                    RaiseLastOpenSslError(EDigestException, FALSE, 'Failed to init EC parameters');
                ret := EVP_PKEY_fromdata(pctx, @Result, ctype, ParamArray);
                if ret = -2 then
                    Raise EDigestException.Create('Parameters not supported for EC keys')
                else if ret < 1 then
                    RaiseLastOpenSslError(EDigestException, FALSE, 'Failed to set EC params');
