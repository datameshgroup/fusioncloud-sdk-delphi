...
Legal issues: Copyright (C) 1999-2021 by François PIETTE
...
Dec 09, 2020 V8.65 Support /.well-known/ from other proxies with absolute URL.
...
May 24, 2021 V8.67 Added optional LastModified param to AnswerStream, AnswerPage,
...
const
    THttpServerVersion = 867;
    CopyRight : String = ' THttpServer (c) 1999-2021 F. Piette V8.67 ';
    DefServerHeader : string = 'Server: ICS-HttpServer-8.67';   { V8.09 }
...
type
    ...
    protected
        ...
        FOnBeforeCtxInit               : TBeforeCtxInitEvent;     { V8.66 }
        ...
        procedure TransferBeforeCtxInit(Sender: TObject; IcsHost: TIcsHost);    { V8.66 }
{$IFDEF AUTO_X509_CERTS}  { V8.59 }
...
{$ENDIF} // AUTO_X509_CERTS
        ...
        property  OnBeforeCtxInit : TBeforeCtxInitEvent    read  FOnBeforeCtxInit
                                                           write FOnBeforeCtxInit;     { V8.66 }
    end;
...
begin
    ...
    if CompressionHandled then begin
        ...
        if ZStreamType <> zsZLib then begin
            ...
            FDocStream.Position := 0; { V8.67 Seek (0, 0); { reset to start }
...
procedure THttpConnection.AnswerStream(
    ...
    const Header   : String;   { Do not use Content-Length               }
    LastModified    : TDateTime = 0); { zero => no Last-Modified header  V8.67  }
var
    ContEncoderHdr  : String ;      { V7.21 }
begin
    Flags := hgWillSendMySelf;
    ContEncoderHdr := '';
    FLastModified := LastModified;       { V8.67  }
    ...
    if FLastModified <> 0 then
        PutStringInSendBuffer ('Last-Modified: ' + RFC1123_Date(FLastModified) + ' GMT' + #13#10);   { V8.67  }
    if ContType = '' then
...
{$ELSE}
        ...
        Stream.Position := 0; { V8.67 Seek(0, 0); }
...
begin
    ...
    AnswerStream(Flags, Status, 'text/html', Header, LastModified);   { V8.67  }
end;
...
{$ENDIF}
  ...
  { check for well-known directory, which we treat outside the normal web root }
    if (FMethod = 'GET') and (FWellKnownPath <> '') then begin
        if Pos (WellKnownDirStr, IcsLowercase(FPath)) >= 1 then begin  { V8.65 allow for absolute Path }
...
begin
    if not Assigned(FDocStream) then begin
        ...
        FDocStream.Position := 0; { V8.67 Seek(0, 0);        { Go to start of stream           }
(*
{$IFDEF USE_SSL}
...
begin
    ...
    (FWSocketServer as TSslWSocketServer).OnBeforeContextInit := TransferBeforeCtxInit;  { V8.66 }
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomSslHttpServer.TransferBeforeCtxInit(Sender: TObject; IcsHost: TIcsHost);    { V8.66 }
begin
    if Assigned(FonBeforeCtxInit) then
        FonBeforeCtxInit(Sender, IcsHost);
end;
