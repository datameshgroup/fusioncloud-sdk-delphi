...
const
    IcsSSLEAYVersion   = 867;
    CopyRight : String = ' IcsSSLEAY (c) 2003-2021 F. Piette V8.67 ';
...
var
 ...
 { V8.34 only use libcrypto-1_1.dll or 3.0 not libeay32.dll which is out of support }
    GSSLEAY_DLL_IgnoreOld       : Boolean = True;  { V8.65 now defaults true since out of support }
 ...
 { V8.38 wintrust stuff for authenticode digital signing checking }
  ...
  { note need ICS prefix since OPENSSL_version_major is an export and we are case insensitive }
    ICS_OPENSSL_VERSION_MAJOR   : Integer = {$IFDEF xYuOpenSSL}YuOpenSSL.OPENSSL_VERSION_MAJOR{$ELSE}0{$ENDIF};           { V8.67 1 or 3, macros in OpenSSL 3.0 }
    ICS_OPENSSL_VERSION_MINOR   : Integer = {$IFDEF xYuOpenSSL}YuOpenSSL.OPENSSL_VERSION_MINOR{$ELSE}0{$ENDIF};           { V8.67 0 >  }
    ICS_OPENSSL_VERSION_PATCH   : Integer = {$IFDEF xYuOpenSSL}YuOpenSSL.OPENSSL_VERSION_PATCH{$ELSE}0{$ENDIF};           { V8.67 0 >  }
    ICS_OPENSSL_VERSION_PRE_RELEASE : String = {$IFDEF xYuOpenSSL}YuOpenSSL.OPENSSL_VERSION_PRE_RELEASE{$ELSE}''{$ENDIF}; { V8.67 0 >  }
...
type
    ...
    TOSSL_PARAM = Tossl_param_st;      { ossl_param_st }
    POSSL_PARAM = ^TOSSL_PARAM;        { ossl_param_st }
...
const
    GSSLEAYImports1: array[0..158] of TOSSLImports = (
