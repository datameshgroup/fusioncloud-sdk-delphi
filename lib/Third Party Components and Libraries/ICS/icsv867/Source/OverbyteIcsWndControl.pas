...
{$ELSE MSWINDOWS}
    ...
    FHandle := Ics.Posix.PXMessages.AllocateHWND(WndProc);  { V8.65 }
    AllocateMsgHandlers;
{$ENDIF}
end;
...
{$ELSE MSWINDOWS}
    FreeMsgHandlers;
    Ics.Posix.PXMessages.DestroyWindow(FHandle);  { V8.65 }
    FHandle := 0;
        ...
        with MsgRec do begin
          ...
          {$ELSE} //Posix
                ...
                if { (not IsBadReadPtr(Obj, GUIDOffSet + SizeOf(INT_PTR))) and }
                   (PUINT_PTR(WParam + Ics.Posix.PXMessages.WPARAM(GUIDOffSet))^ = WParam) and { V8.65 }
                ...
                Obj := TObject(WParam);
                if {(not IsBadReadPtr(Obj, GUIDOffSet + SizeOf(INT_PTR))) and }
                   (PINT_PTR(WParam + Ics.Posix.PXMessages.WPARAM(GUIDOffSet))^ = LParam) and  { V8.65 }
