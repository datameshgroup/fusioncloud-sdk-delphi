 ...
 *
 *  CHANGES:
 *  Apr 25, 2018, created ICS version for Json applications in older versions
 ...
 *  Aug 11, 2020 - V8.65 Increased SUPER_TOKENER_MAX_DEPTH from 32 to 64 to allow
 *                     parsing of more complex Json files, uses more memory.
 *                 Added ParseStringEx and SOex which return an error string with the
 *                    first parse error rather than just returning nil.
 ...
 *                 Removed JavaToDelphiDateTime and DelphiToJavaDateTime which worked
 *                    with Unix time and used a lot of oode.
 *                 Removed TSuperWriterSock, use WSocket instead.
 *                 Use AtomicIncrement instead of InterlockedIncrement.
 *                 Added new datetime get/set type for Delphi TDateTime, saves as
 8                   as ISO 8601/RFC3339 string:
 *                     obj.AsDateTime
 *                     obj.AsObject.DT['foo']
 *                     obj.DT['foo']
 *                     obj.AsArray.DT[0]
...
// do we need FormatSettings.DecimalSeparator
{$IFDEF COMPILER16_UP)
...
type
{$IFNDEF FPC}
  PtrInt = Integer;   { V8.65 was longint }
  PtrUInt = Longword;
{$ENDIF}
  SuperInt = Int64;
...
const
  ...
  SUPER_TOKENER_MAX_DEPTH = 64;        { V8.65 was 32, should make it dynamic }

  SUPER_AVL_MAX_DEPTH = sizeof(Integer) * 8;        { V8.65 was longint }
...
{$ENDIF}
    ...
    function GetDT(const k: SOString): TDateTime;            { V8.65 }
    procedure PutDT(const k: SOString; Value: TDateTime);    { V8.65 }
  public
...
{$ENDIF}
    ...
    property DT[const k: SOString]: TDateTime read GetDT write PutDT;   { V8.65 }
...
{$ENDIF}
    ...
    function GetDT(const index: integer): TDateTime;            { V8.65 }
    procedure PutDT(const index: integer; Value: TDateTime);    { V8.65 }
  public
    constructor Create; virtual;
    destructor Destroy; override;
...
{$ENDIF}
    ...
    function GetDT(const path: SOString): TDateTime;            { V8.65 }
    procedure PutDT(const path: SOString; Value: TDateTime);    { V8.65 }
...
//    function SaveTo(socket: longint; indent: boolean = false; escape: boolean = true): integer; overload;  { V8.65 gone}
...
{$ENDIF}
    ...
    function AsDateTime: TDateTime;                               { V8.65 }
...
{$ENDIF}
    ...
    property DT[const path: SOString]: TDateTime read GetDT write PutDT;   { V8.65 }
...
//  function SaveTo(socket: longint; indent: boolean = false; escape: boolean = true): integer; overload;  { V8.65 gone }
    ...
    class function ParseStringEx(s: PSOChar; strict: Boolean; var ErrStr: String): ISuperObject;   { V8.65 }
...
{$ENDIF}
    function AsDateTime: TDateTime; virtual;             { V8.65 }
    procedure Clear(all: boolean = false); virtual;
...
function SOex(const s: SOString; var ErrStr: String): ISuperObject; overload;  { V8.65 }
...
{$I Include\Ics.InterlockedApi.inc}    { V8.65 use this instead of Winapi.Windows } 
...
function SOex(const s: SOString; var ErrStr: String): ISuperObject; overload;  { V8.65 }
begin
  Result := TSuperObject.ParseStringEx(PSOChar(s), False, ErrStr);
end;
...
//    varDate:     Result := TSuperObject.Create(DelphiToJavaDateTime(vDate));
    varDate:     Result := TSuperObject.Create(SOString(RFC3339_DateToStr(vDate, True)));   { V8.65 Json dates are strings }
...
begin
    Result := TSuperObject.Create(RFC1123_StrToDate(TValueData(value).FAsString, True));      { V8.65 }
end;
...
{  stInt:
  ...
  stString:                                          { V8.65 only string dates }
    begin
      dt := RFC3339_StrToDate(obj.AsString, True);
...
begin
  inherited Create;
{$IFDEF DEBUG}
  AtomicIncrement(debugcount);  { V8.65 was InterlockedIncrement }
{$ENDIF}
...
begin
{$IFDEF DEBUG}
  AtomicDecrement(debugcount);    { V8.65 was InterlockedDecrement }
{$ENDIF}
  case FDataType of
...
begin
  ...
  //     Result := Append(PSOChar(SOString(gcvt(FO.c_double, 15, fbuffer))));
            Result := Append(PSOChar(SOString(FloatToStrF(FO.c_double, ffFixed, 15, 10))));  { V8.65 was gcvt }
      stCurrency:
        begin
...
function TSuperObject.AsDateTime: TDateTime;                               { V8.65 }
begin
  if FDataType = stString then
    Result := RFC3339_StrToDate(FOString, True)
  else
    Result := 0;
end;
...
begin
  AtomicDecrement(FRefCount);       { V8.65 was InterlockedDecrement }
end;
...
begin
  ...
  if(tok.err <> teSuccess) or (not partial and (s[tok.char_offset] <> #0)) then begin
    Result := nil;
  end
  else
    Result := obj;
...
class function TSuperObject.ParseStringEx(s: PSOChar; strict: Boolean; var ErrStr: String): ISuperObject;   { V8.65 }
var
  tok: TSuperTokenizer;
  obj: ISuperObject;
begin
  ErrStr := '';
  tok := TSuperTokenizer.Create;
  obj := ParseEx(tok, s, -1, strict, Nil, [], nil, stNull);
  if(tok.err <> teSuccess) or (s[tok.char_offset] <> #0) then begin
     Result := nil;
     ErrStr := 'Error ' + GetEnumName(TypeInfo(TSuperTokenizerError), Ord(tok.err)) +
            ' at line ' + IntToStr(tok.Line) +  ' column ' + IntToStr(tok.col) +
            ' at depth ' + IntToStr(tok.depth) ;            { V8.65 report parsing error }
  end
  else
    Result := obj;
  tok.Free;
...
//    class function ParseStreamEx(stream: TStream; strict: Boolean; var ErrStr: String): ISuperObject;   { V8.65 }
...
begin
  ...
  begin
    ...
    begin
      unicode := false;
      stream.Position := 0; { V8.67 Seek(0, soFromBeginning); }
  ...
  if(tok.err <> teSuccess) or (not partial and (st[tok.char_offset] <> #0)) then begin
    Result := nil;
    errmsg := 'Error ' + GetEnumName(TypeInfo(TSuperTokenizerError), Ord(tok.err)) +
            ' at line ' + IntToStr(tok.Line) +  ' column ' + IntToStr(tok.col) +
            ' at depth ' + IntToStr(tok.depth) ;            { V8.65 report parsing error }
  end
  else
  ...
  tok.Free;
  if errmsg <> '' then
        raise Exception.Create(errmsg);   { V8.65 }
end;
...
procedure TSuperObject.PutDT(const path: SOString; Value: TDateTime);   { V8.65 }
begin
  ParseString(PSOChar(path), true, False, self, [foCreatePath, foPutValue], TSuperObject.Create(RFC3339_DateToStr(Value, True)));
end;
...
function TSuperObject.GetDT(const path: SOString): TDateTime;  { V8.65 }
var
  obj: ISuperObject;
begin
  obj := GetO(path);
  if obj <> nil then
    Result := obj.AsDateTime else
    Result := 0;
end;
...
begin
  Result := AtomicIncrement(FRefCount);   { V8.65 was InterlockedIncrement }
end;

function TSuperObject._Release: Integer; stdcall;
begin
  Result := AtomicDecrement(FRefCount);  { V8.65 was InterlockedDecrement }
  if Result = 0 then
    Destroy;
end;
...
procedure TSuperArray.PutDT(const index: integer; Value: TDateTime);   { V8.65 }
begin
  PutO(index, TSuperObject.Create(RFC3339_DateToStr(Value, True)));
end;
...
function TSuperTableString.GetDT(const k: SOString): TDateTime;   { V8.65 }
var
  obj: ISuperObject;
begin
 obj := GetO(k);
...
procedure TSuperTableString.PutDT(const k: SOString; value: TDateTime);   { V8.65 }
begin
  PutO(k, TSuperObject.Create(RFC3339_DateToStr(Value, True)));
end;
