...
uses
  ...
  OverbyteIcsFtpCli, OverbyteIcsIpStreamLog;
...
type
  ...
  TBinClientForm = class(TForm)
    ...
    Btn2: TButton;
    SslFtpClient1: TSslFtpClient;
    SslContext1: TSslContext;
    WSocket1: TWSocket;
    IcsIpStrmLog1: TIcsIpStrmLog;
...
procedure TBinClientForm.Btn1Click(Sender: TObject);
var
  Client: TWSocketClient;
begin
//  WSocket1.Connect;
  Client := TWSocketClient.Create(self);
  Client.OnSessionConnected := OnConnect;
  Client.CServerAddr := 'wss://www.cloudposintegration.io/nexouat1';
  Client.StartConnection;

//  Sock1.Connect;

//  IcsIpStrmLog1.Socket.Connect;

//  SslHttpCli1.URL := 'wss://www.cloudposintegration.io/nexouat1';
//  SslHttpCli1.Get;
end;

procedure TBinClientForm.Btn2Click(Sender: TObject);
begin
  case SslHttpCli1.State of
    httpReady: ShowMessage('ready');
    httpNotConnected: ShowMessage('not connected');
    httpConnected: ShowMessage('connected');
    httpDnsLookup: ShowMessage('dns lookup');
    httpDnsLookupDone: ShowMessage('look up done');
    httpWaitingHeader: ShowMessage('waiting header');
    httpWaitingBody: ShowMessage('waiting body');
    httpBodyReceived: ShowMessage('received');
    httpWaitingProxyConnect: ShowMessage('proxy connect');
  end;
end;
...
procedure TBinClientForm.OnConnect(ASender: TObject; AErrCode: Word);
begin
