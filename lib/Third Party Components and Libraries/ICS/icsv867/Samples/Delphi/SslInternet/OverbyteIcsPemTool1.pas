...
const
     PemToolVersion     = 867;
     PemToolDate        = 'September 27, 2021';
     PemToolName        = 'PEM Certificate Tool';
     CopyRight : String = '(c) 2003-2021 by François PIETTE V8.67 ';
...
begin
    ...
    if Assigned(RootCAList) then  RootCAList.Free;
...
begin
    ...
    try
        try
            ...
            if Errs <> '' then
                frmPemTool2.Memo1.Lines.Add (Errs);   { V8.65 }
...
begin
    ...
    try
        ...
        if Errs <> '' then
             frmPemTool2.Memo1.Lines.Add ('Errors reading bundle file: ' + Errs);  { V8.65 }
...
procedure TfrmPemTool1.ComboBox1Change(Sender: TObject);
begin
...
var
    ...
    Errs: String;
begin
    if NOT Assigned(RootCAList) then
        RootCAList := TX509List.Create(Self);
    RootCAList.Clear;
    Internal := False;
...
begin
    LoadRootCA;     { V8.65 }
    ShowLogWindow;
    frmPemTool2.Memo1.Lines.Add ('CA root bundle certificate file ' + CAFilesDir.Text);
    frmPemTool2.Memo1.Lines.Add ('Number of root certificates found: ' + IntToStr (RootCAList.Count));
    if RootCAList.Count > 0 then begin
...
begin
    ...
    OpenDlg.InitialDir := ExtractFileDir(FileName);
    ShowLogWindow;
    frmPemTool2.Memo1.Lines.Add('Resaving file with ' + SslPrivKeyCipherLits[
    ...
    begin
        frmPemTool2.Memo1.Lines.Add(ErrMsg);
    end
    else
        frmPemTool2.Memo1.Lines.Add(ErrMsg);
end;
...
begin
    ...
    ShowLogWindow;
    frmPemTool2.Memo1.Lines.Add ('Microsoft Certificate Store: ' + MsStoreType.Items[MsStoreType.ItemIndex] +
                                                                ' - ' + MsLocationType.Items[MsLocationType.ItemIndex]);
    CertStoreList.Items.Clear;
    if FMsX509List.MsLastError <> '' then
         frmPemTool2.Memo1.Lines.Add ('Errors reading store: ' + FMsX509List.MsLastError);
    if Total <= 0 then begin
        frmPemTool2.Memo1.Lines.Add ('No certificates found in store');
    end
    else begin
        frmPemTool2.Memo1.Lines.Add ('Number of certificates found in store: ' + IntToStr (Total));
        for I := 0 to Total - 1 do begin
           ...
           if FMsX509List [I].IsPKeyLoaded then begin
                if (FMsX509List [I].CheckCertAndPKey) then
                    frmPemTool2.Memo1.Lines.Add ('!! Private key available for certificate: ' + FMsX509List [I].KeyInfo)
                else
                    frmPemTool2.Memo1.Lines.Add ('!! Private key available but does not match certificate');
            end;
        end;
    end;
    frmPemTool2.Memo1.Lines.Add ('');
...
begin
    ShowLogWindow;
    if CertStoreList.ItemIndex < 0 then begin
        frmPemTool2.Memo1.Lines.Add('No certificate store item selected');
        Exit;
    end;
    try
        ...
        if FMsX509List.DeleteFromStore(FMsX509List [CertStoreList.ItemIndex].Sha1Digest,
            ...
            frmPemTool2.Memo1.Lines.Add('Deleted cerfificate from store OK - ' + CName);
            frmPemTool2.Memo1.Lines.Add ('');
            doMsDisplayClick(Self);  // refresh store window
        end ;
        if FMsX509List.MsLastError <> '' then
            frmPemTool2.Memo1.Lines.Add('Failed to delete cerfificate from store - ' + FMsX509List.MsLastError);
    except
        on E:Exception do
            frmPemTool2.Memo1.Lines.Add(E.Message);
    end;
...
begin
    ...
    if (NOT IcsIsProgAdmin) and (TMsCertLocation(MsLocationType.ItemIndex) = MsLocMachine) then begin
        frmPemTool2.Memo1.Lines.Add ('Program needs administrator rights to access local machine store');
        Exit;
    end;
    ...
    try
        ...
        frmPemTool2.Memo1.Lines.Add ('Microsoft Certificate Personal/My Store - via PFX/P12');
        if NOT MsCertTools.IsCertLoaded then begin
            ...
            if MsCertTools.IsPKeyLoaded then begin
                if (MsCertTools.CheckCertAndPKey) then
                    frmPemTool2.Memo1.Lines.Add ('!! Private key available for certificate: ' + MsCertTools.KeyInfo)
                else
                    frmPemTool2.Memo1.Lines.Add ('!! Private key available but does not match certificate');
            end;
...
begin
    ...
    if NOT FileExists(MsInstallFile.Text) then
        frmPemTool2.Memo1.Lines.Add ('Certificates file not found: ' + MsInstallFile.Text)
    else begin
        MsCertTools := TMsCertTools.Create(self);
        try
            try
                ...
                frmPemTool2.Memo1.Lines.Add ('Saved certificate to Windows Store OK');
                frmPemTool2.Memo1.Lines.Add ('');
                doMsDisplayClick(Self);  // refresh store window
            except
                on E:Exception do frmPemTool2.Memo1.Lines.Add (E.Message);
            end;
        finally
            MsCertTools.Free;
         end;
...
begin
    ...
    FStorePkeyTot := 0;
    if (NOT IcsIsProgAdmin) and (TMsCertLocation(MsLocationType.ItemIndex) = MsLocMachine) then begin
        frmPemTool2.Memo1.Lines.Add ('Program needs administrator rights to access local machine keys');
        Exit;
    end;
    if NOT Assigned(FMsX509List) then
        FMsX509List := TMsX509List.Create(nil, True);
    ...
    ShowLogWindow;
    frmPemTool2.Memo1.Lines.Add ('Microsoft Key Store: ' + MsLocationType.Items[MsLocationType.ItemIndex]);
    if FMsX509List.MsLastError <> '' then
         frmPemTool2.Memo1.Lines.Add ('Errors reading key store: ' + FMsX509List.MsLastError);
    if FStorePkeyTot <= 0 then begin
        frmPemTool2.Memo1.Lines.Add ('No private keys found in store');
    end
    else begin
        frmPemTool2.Memo1.Lines.Add ('Number of private keys found in store: ' + IntToStr (FStorePkeyTot));
...
begin
    ...
    if PkeyList.ItemIndex < 0 then begin
        frmPemTool2.Memo1.Lines.Add('No private key store item selected');
        Exit;
    end;
    try
        ...
        if FMsX509List.DeletePKey(CName, TMsCertLocation(MsLocationType.ItemIndex)) then begin
            frmPemTool2.Memo1.Lines.Add('Deleted private key from store OK - ' + CName);
            frmPemTool2.Memo1.Lines.Add ('');
            doListPkeysClick(Self);  // refresh store window
        end ;
        if FMsX509List.MsLastError <> '' then
            frmPemTool2.Memo1.Lines.Add('Failed to delete private key from store - ' + FMsX509List.MsLastError);
    except
        on E:Exception do
            frmPemTool2.Memo1.Lines.Add(E.Message);
    end;
