...
Delphi editions.  This also requires DISQLite3 to be installed from http://www.yunqa.de,
and for the file World.db3 to be copied from the DISQLite3\Demos into the SslInternet
folder.
...
{$IFDEF DATABASE}
    ...
    DISQLite3Api,                        // DISQLite from http://www.yunqa.de
    DISQLite3Database,
    DISQLite3DataSet,
{$ENDIF}
  OverbyteIcsWSocket,
...
const
    SrvCopyRight : String = ' OverbyteIcsDDWebService (c) 2021 Francois Piette V8.67 ';
    MaxWinChars = 800000;
...
begin
    ClientCnx.AnswerString(Flags,
        ...
        '<HTML>' +
          '<HEAD>' +
            '<TITLE>' +
                'ICS DD Web Service Demo - Password Protected Page - Digest' +
            '</TITLE>' +
          '</HEAD>' +
          '<BODY>' +
            '<H2>ICS DD Web Service Password Protected Page</H2>' +
 ...
 // open
    ...
    if (sCountry <> '') and DbOpened then begin
        ...
        if DbOpened and (JsonResp  = '') then begin  // no error yet
            try
                if NOT RestParamsSrv.ResultSet2Json(DISQLite3Query, JsonResp) then
...
{$ELSE}  // no datanase, simple Json response
    ...
    JsonResp := String(JsonResult.GetParameters);
    JsonResult.Free;
{$ENDIF}
