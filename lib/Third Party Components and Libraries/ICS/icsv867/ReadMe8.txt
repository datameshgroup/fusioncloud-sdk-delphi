...
Legal issues:
-------------
              Copyright (C) 1997-2021 by François PIETTE
...
Testing of ICS is primarily with Windows 10 and Server 2019, also Windows 7 and Servers
2008 R2 and 2012, note that Windows XP is not supported and SSL will not work. Initial
testing has been done on Windows 11 and Server 2022.
...
OpenSSL 1.1.1 files in the .\OpenSSL-Win32 directory and the four main DLLs duplicated
in .\Samples\delphi\sslinternet for Win32 and Win64 samples.
...
OpenSSL has a new version 3.0 with newly named files, libcrypto-3.dll (or
libcrypto-3-x64.dll) and libssl-3.dll (or libssl-3-x64).dll, which may is also
available from the ICS Download site, and requires ICS V8.67 or later to be used.
...
are needed unless you check or report the DLL direc
...
C++ Builder 11.0 FMX    : IcsFmxCB110Run.dproj, IcsFmxCB110Design.dproj
C++ Builder 11.0 VCL    : IcsVclCB110Run.dproj, IcsVclCB110Design.dproj
...
Note following sample is not in the project group since it requires DDService framework to be installed
> OverbyteIcsDDWebService.dpr     Advanced Windows Service multi host web server demom, maybe run as a GUI   - ACTIVE!!
...
requires DISQLite3 to be installed. Note this sample in not in the project
groups due to these pre-requisites.
