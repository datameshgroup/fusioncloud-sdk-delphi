unit DataMeshGroup.Fusion.SaleToPOIMessage;

interface

uses DataMeshGroup.Fusion.MessageHeader, DataMeshGroup.Fusion.MessagePayload,
  DataMeshGroup.Fusion.SecurityTrailer;

type
  TSaleToPOIMessage = class
  private
    FMessageHeader: TMessageHeader;
    FMessagePayload: TMessagePayload;
    FSecurityTrailer: TSecurityTrailer;
  public
    property MessageHeader: TMessageHeader read FMessageHeader write FMessageHeader;
    property MessagePayload: TMessagePayload read FMessagePayload
      write FMessagePayload;
    property SecurityTrailer: TSecurityTrailer read FSecurityTrailer
      write FSecurityTrailer;
  end;

implementation

end.
